    ...
    >
        <div class="flex justify-between text-right">
            <p class="text-base max-md:font-normal max-sm:text-sm">
                @{{ cart.coupon_code ? "@lang('lord::app.checkout.coupon.applied')" : "@lang('lord::app.checkout.coupon.discount')" }}
            </p>

            {!! view_render_event('bagisto.lord.checkout.cart.coupon.before') !!}
            ...
            <p class="text-base font-medium max-sm:text-sm">
                ...
                >
                    ...
                    <form @submit="handleSubmit($event, applyCoupon)">
                        {!! view_render_event('bagisto.lord.checkout.cart.coupon.coupon_form_controls.before') !!}

                        <!-- Apply coupon modal -->
                        <x-lord::modal ref="couponModel">
                            <!-- Modal Toggler -->
                            <x-slot:toggle>
                                <span 
                                ...
                                >
                                    @lang('lord::app.checkout.coupon.apply')
                                </span>
                            </x-slot>
                            ...
                            <x-slot:content class="!px-4">
                                <x-lord::form.control-group class="!mb-0">
                                    <x-lord::form.control-group.control
                                        ...
                                        name="code"
                                        rules="required"
                                        :placeholder="trans('lord::app.checkout.coupon.enter-your-code')"
                                    />
                                ...
                                <div class="flex flex-wrap items-center gap-4 max-md:justify-between">
                                    <div class="flex items-center gap-4 max-md:block">
                                        <p class="text-sm font-medium text-zinc-500 max-md:text-left max-md:text-xs">
                                            @lang('lord::app.checkout.coupon.subtotal')
                                        </p>
                                    ...
                                    <x-lord::button
                                        ...
                                        :title="trans('lord::app.checkout.coupon.button-title')"
                                        ::loading="isStoring"
                                        ::disabled="isStoring"
                                    />
                                </div>
                        ...
                        {!! view_render_event('bagisto.lord.checkout.cart.coupon.coupon_form_controls.after') !!}
                    </form>
                </x-lord::form>
                ...
                >
                    <p 
                        ...
                        title="@lang('lord::app.checkout.coupon.applied')"
                    >
                        "@{{ cart.coupon_code }}"
                    </p>
                    ...
                    <span 
                        ...
                        title="@lang('lord::app.checkout.coupon.remove')"
                        @click="destroyCoupon"
                    >
                    </span>
                </div>
            ...
            {!! view_render_event('bagisto.lord.checkout.cart.coupon.after') !!}
        </div>
    </script>
        ...
        app.component('v-coupon', {
            ...
            methods: {
                applyCoupon(params, { resetForm }) {
                    ...
                    this.$axios.post("{{ route('lord.api.checkout.cart.coupon.apply') }}", params)
                        .then((response) => {
                ...
                destroyCoupon() {
                    this.$axios.delete("{{ route('lord.api.checkout.cart.coupon.remove') }}", {
                            '_token': "{{ csrf_token() }}"
                        })
                        .then((response) => {
