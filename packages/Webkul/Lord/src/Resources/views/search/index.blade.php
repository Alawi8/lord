@php
    $title = request()->has('query')
            ? trans('lord::app.search.title', ['query' => request()->query('query')])
            : trans('lord::app.search.results');
@endphp
...
<x-lord::layouts :has-feature="false">
    ...
    <div class="container px-[60px] max-lg:px-8 max-sm:px-4">
        @if (request()->has('image-search'))
            @include('lord::search.images.results')
        @endif
                ...
                <div class="flex items-start gap-10 max-lg:gap-5 md:mt-10">
                    ...
                    <div class="flex-1">
                                ...
                                <template v-if="products.length">
                                    <div class="mt-8 grid grid-cols-3 gap-8 max-1060:grid-cols-2 max-md:mt-5 max-md:justify-items-center max-md:gap-x-4 max-md:gap-y-5">
                                        <x-lord::products.card
                                            ::mode="'grid'"
                                            v-for="product in products"
                                            :navigation-link="route('lord.search.index')"
                                        />
                                    </div>
                                </template>
                        ...
                        <button
                            ...
                            v-if="links.next"
                        >
                            @lang('lord::app.categories.view.load-more')
                        </button>
                    </div>
                </div>
            </div>
            ...
            app.component('v-search', {
                ...
                methods: {
                    ...
                    getProducts() {
                        ...
                        this.$axios.get(("{{ route('lord.api.products.index') }}"), {
