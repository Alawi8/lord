<!-- SEO Meta Content -->
@push('meta')
    <meta name="description" content="@lang('lord::app.compare.title')"/>

    <meta name="keywords" content="@lang('lord::app.compare.title')"/>
@endPush

<x-lord::layouts>
    <!-- Page Title -->
    <x-slot:title>
        @lang('lord::app.compare.title')
    </x-slot>

    <!-- Breadcrumb -->
    @if ((core()->getConfigData('general.general.breadcrumbs.lord')))
        <div class="mt-5 flex justify-center max-lg:hidden">
            {!! view_render_event('bagisto.lord.customers.account.compare.breadcrumbs.before') !!}

            <div class="flex items-center gap-x-2.5">
                <x-lord::breadcrumbs name="compare" />
            </div>

            {!! view_render_event('bagisto.lord.customers.account.compare.breadcrumbs.after') !!}
        </div>
    @endif
    ...
    <div class="container mt-8 px-[60px] max-lg:px-8 max-md:mt-7 max-md:px-0">
        <v-compare>
            <!-- Shimmer Effect -->
            <x-lord::shimmer.compare :attributeCount="count($comparableAttributes)" />
        </v-compare>
    </div>
    ...
    @pushOnce('scripts')
        ...
        >
            <div>
                {!! view_render_event('bagisto.lord.customers.account.compare.before') !!}

                <div v-if="! isLoading">
                    <div class="flex items-center justify-between max-md:px-4">

                        {!! view_render_event('bagisto.lord.customers.account.compare.title.before') !!}

                        <h1 class="text-2xl font-medium max-sm:text-base">
                            @lang('lord::app.compare.title')
                        </h1>

                        {!! view_render_event('bagisto.lord.customers.account.compare.title.after') !!}

                        {!! view_render_event('bagisto.lord.customers.account.compare.remove_all.before') !!}
                        ...
                        >
                            ...
                            @lang('lord::app.compare.delete-all')
                        </div>

                        {!! view_render_event('bagisto.lord.customers.account.compare.remove_all.after') !!}
                    </div>
                    ...
                    >
                        <template v-for="attribute in comparableAttributes">
                            ...
                            >
                                {!! view_render_event('bagisto.lord.customers.account.compare.attribute_name.before') !!}
                                ...
                                {!! view_render_event('bagisto.lord.customers.account.compare.attribute_name.after') !!}
                                ...
                                <div class="flex gap-3 border-zinc-200 max-md:gap-0 max-md:border-0 ltr:border-l-[1px] rtl:border-r-[1px]">
                                    ...
                                    >
                                        ...
                                        <x-lord::products.card class="[&_span.icon-compare]:hidden" />
                                    </div>
                                </div>
                            </div>

                            {!! view_render_event('bagisto.lord.customers.account.compare.comparable_attribute.before') !!}
                            ...
                            {!! view_render_event('bagisto.lord.customers.account.compare.comparable_attribute.after') !!}
                        </template>
                    </div>
                    ...
                    >
                        <img
                            ...
                            alt="@lang('lord::app.compare.empty-text')"
                        />
                        ...
                        <p
                            ...
                            role="heading"
                        >
                            @lang('lord::app.compare.empty-text')
                        </p>
                    </div>
                </div>
                ...
                {!! view_render_event('bagisto.lord.customers.account.compare.after') !!}
            </div>
        </script>
            ...
            app.component("v-compare", {
                ...
                methods: {
                    getItems() {
                        ...
                        this.$axios.get("{{ route('lord.api.compare.index') }}", {
                                params: {
                    ...
                    remove(productId) {
                        this.$emitter.emit('open-confirm-modal', {
                            agree: () => {
                                ...
                                this.$axios.post("{{ route('lord.api.compare.destroy') }}", {
                                        '_method': 'DELETE',
                        ...
                        this.$emitter.emit('open-confirm-modal', {
                            agree: () => {
                                if (! this.isCustomer) {
                                    ...
                                    this.$emitter.emit('add-flash', { type: 'success', message:  "@lang('lord::app.compare.remove-all-success')" });

                                    return;
                                }
                                
                                this.$axios.post("{{ route('lord.api.compare.destroy_all') }}", {
                                        '_method': 'DELETE',
                                    })
                                    .then(response => {
                                        this.items = [];
