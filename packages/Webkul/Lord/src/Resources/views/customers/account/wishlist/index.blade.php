<x-lord::layouts.account>
    <!-- Page Title -->
    <x-slot:title>
        @lang('lord::app.customers.account.wishlist.page-title')
    </x-slot>

    <!-- Breadcrumbs -->
    @if ((core()->getConfigData('general.general.breadcrumbs.lord')))
        @section('breadcrumbs')
            <x-lord::breadcrumbs name="wishlist" />
        @endSection
    @endif
    ...
    <div class="mx-4 flex-auto">
        ...
        <v-wishlist-products>
            ...
            <x-lord::shimmer.customers.account.wishlist :count="4" />
        </v-wishlist-products>
    </div>
    ...
    @pushOnce('scripts')
        ...
        >
            <div>
                ...
                {!! view_render_event('bagisto.lord.customers.account.wishlist.list.before') !!}
                ...
                <template v-else>
                    <div class="flex items-center justify-between">
                        ...
                        {!! view_render_event('bagisto.lord.customers.account.wishlist.delete_all.before') !!}
                        ...
                        >
                            @lang('lord::app.customers.account.wishlist.delete-all')
                        </div>

                        {!! view_render_event('bagisto.lord.customers.account.wishlist.delete_all.after') !!}
                    </div>
                    ...
                    <template v-if="wishlistItems.length">
                        ...
                        >
                            <x-lord::shimmer.customers.account.wishlist />
                        </v-wishlist-products-item>
                    </template>
                    ...
                    <template v-else>
                        <div class="m-auto grid w-full place-content-center items-center justify-items-center py-32 text-center">
                            ...
                            <p
                                ...
                                role="heading"
                            >
                                @lang('lord::app.customers.account.wishlist.empty')
                            </p>
                        </div>
                    </template>
                </template>

                {!! view_render_event('bagisto.lord.customers.account.wishlist.list.after') !!}
            </div>
        </script>
                    ...
                    <div class="grid gap-y-6 max-md:gap-y-0">
                        ...
                        <div class="flex justify-between gap-x-2.5 border-b border-zinc-200 pb-5">
                            <div class="flex gap-x-5 max-md:w-full max-md:gap-x-5">
                                <div>
                                    {!! view_render_event('bagisto.lord.customers.account.wishlist.image.before') !!}

                                    <a :href="`{{ route('lord.product_or_category.index', '') }}/${wishlist.product.url_key}`">
                                        <!-- Wishlist Item Image -->
                                        <img
                                    ...
                                    {!! view_render_event('bagisto.lord.customers.account.wishlist.image.after') !!}
                                </div>
                                ...
                                <div class="grid gap-y-2.5 max-md:w-full max-md:gap-y-0">
                                    ...
                                    >
                                        <div class="grid gap-2">
                                            <div>
                                                ...
                                                >
                                                    @lang('lord::app.customers.account.wishlist.see-details')
                                    ...
                                    <div class="max-md:block md:hidden">
                                        ...
                                        {!! view_render_event('bagisto.lord.customers.account.wishlist.remove_button.before') !!}
                                        ...
                                        <a
                                            ...
                                            @click="remove"
                                        >
                                            @lang('lord::app.customers.account.wishlist.remove')
                                        </a>

                                        {!! view_render_event('bagisto.lord.customers.account.wishlist.remove_button.after') !!}
                                    </div>

                                    {!! view_render_event('bagisto.lord.customers.account.wishlist.perform_actions.before') !!}
                                    ...
                                    <div class="flex gap-5 max-md:mt-2.5">
                                        ...
                                        @if (core()->getConfigData('sales.checkout.lordping_cart.cart_page'))
                                            ...
                                            <x-lord::button
                                                ...
                                                :title="trans('lord::app.customers.account.wishlist.move-to-cart')"
                                    ...
                                    {!! view_render_event('bagisto.lord.customers.account.wishlist.perform_actions.after') !!}
                                </div>
                            </div>
        ...
        <script type="module">
            app.component("v-wishlist-products", {
                ...
                methods: {
                    get() {
                        this.$axios.get("{{ route('lord.api.customers.account.wishlist.index') }}")
                            .then(response => {
                    ...
                    removeAll() {
                        this.$emitter.emit('open-confirm-modal', {
                            agree: () => {
                                this.$axios.post("{{ route('lord.api.customers.account.wishlist.destroy_all') }}", {
                                        '_method': 'DELETE',
                                    })
                                    .then(response => {
                                        this.wishlistItems = [];
                ...
                methods: {
                    remove() {
                        this.$emitter.emit('open-confirm-modal', {
                            agree: () => {
                                this.$axios
                                    .delete(`{{ route('lord.api.customers.account.wishlist.destroy', '') }}/${this.wishlist.id}`)
                                    .then(response => {
                    ...
                    moveToCart() {
                        ...
                        const endpoint = `{{ route('lord.api.customers.account.wishlist.move_to_cart', ':wishlistId:') }}`.replace(':wishlistId:', this.wishlist.id);
