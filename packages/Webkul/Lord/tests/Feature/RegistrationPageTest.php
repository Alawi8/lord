...
use Webkul\Lord\Mail\Customer\RegistrationNotification as LordRegistrationNotification;
...
it('returns a successful response', function () {
    // Act and Assert.
    get(route('lord.customers.register.index'))
        ->assertOk()
        ->assertSeeText(trans('lord::app.customers.signup-form.page-title'));
});

it('should fails validation error when certain inputs are invalid when register', function () {
    // Act and Assert.
    postJson(route('lord.customers.register.store'))
...
it('successfully registers a customer', function () {
    ...
    // Act and Assert.
    post(route('lord.customers.register.store'), $requestedCustomer)
        ->assertRedirectToRoute('lord.customer.session.index')
        ->assertSessionHas('success', trans('lord::app.customers.signup-form.success'));
});
...
it('successfully registers a customer and send mail to the customer verify the account', function () {
    ...
    post(route('lord.customers.register.store'), $requestedCustomer)
        ->assertRedirectToRoute('lord.customer.session.index')
        ->assertSessionHas('success', trans('lord::app.customers.signup-form.success-verify'));
...
it('registers a customer successfully and sends a registration email to customer and admin along with a success message', function () {
    ...
    Mail::assertQueued(LordRegistrationNotification::class);
