...
use Webkul\Lord\Mail\Customer\SubscriptionNotification;
...
it('displays the "Sign In" and "Sign Up" buttons when the customer is not logged in', function () {
     ...
     * produce false positive results when dealing with large DOM sizes.
     */
    expect(Str::contains($response->content(), trans('lord::app.components.layouts.header.desktop.bottom.sign-in')))
        ->toBeTruthy();

    expect(Str::contains($response->content(), trans('lord::app.components.layouts.header.desktop.bottom.sign-up')))
        ->toBeTruthy();
});
...
it('displays navigation buttons when the customer is logged in', function () {
     ...
     * produce false positive results when dealing with large DOM sizes.
     */
    expect(Str::contains($response->content(), trans('lord::app.components.layouts.header.desktop.bottom.profile')))
        ->toBeTruthy();

    expect(Str::contains($response->content(), trans('lord::app.components.layouts.header.desktop.bottom.orders')))
        ->toBeTruthy();

    expect(Str::contains($response->content(), trans('lord::app.components.layouts.header.desktop.bottom.wishlist')))
        ->toBeTruthy();

    expect(Str::contains($response->content(), trans('lord::app.components.layouts.header.desktop.bottom.logout')))
        ->toBeTruthy();
});
...
it('should returns the search page of the products', function () {
    ...
    // Act and Assert.
    get(route('lord.search.index', [
        'query' => $query = $product->name,
    ]))
        ->assertOk()
        ->assertSeeText($query)
        ->assertSeeText(trans('lord::app.search.title', ['query' => $query]));
});

it('should fails the validation error when provided wrong email address when subscribe to the lord', function () {
    // Act and Assert.
    postJson(route('lord.subscription.store'))
        ->assertJsonValidationErrorFor('email')
        ->assertUnprocessable();
});

it('should store the subscription of the lord', function () {
    // Act and Assert.
    postJson(route('lord.subscription.store'), [
...
it('should store the subscription of the lord and send the mail to the admin', function () {
    // Act and Assert.
    Mail::fake();
...
it('should unsubscribe from the lord', function () {
    // Arrange.
    $subscriber = SubscribersList::factory()->create();

    // Act and Assert.
    get(route('lord.subscription.destroy', [
...
it('should store the products to the compare list', function () {
    ...
    postJson(route('lord.api.compare.store'), [
        'product_id' => $product->id,
    ])
        ->assertOk()
        ->assertSeeText(trans('lord::app.compare.item-add-success'));
});
...
it('should remove product from compare list', function () {
    ...
    deleteJson(route('lord.api.compare.destroy'), [
        'product_id' => $product->id,
    ])
        ->assertOk()
        ->assertJsonPath('message', trans('lord::app.compare.remove-success'));
});
...
it('should remove all the products from compare list', function () {
    ...
    deleteJson(route('lord.api.compare.destroy_all'))
        ->assertOk()
        ->assertJsonPath('data.message', trans('lord::app.compare.remove-all-success'));
});
