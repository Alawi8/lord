...
test("should edit a profile", async ({ page }) => {
    ...
    await page.getByLabel("lord::app.customers.account.").selectOption("Male");
...
test("should able to download downloadable orders", async ({ lordPage }) => {
    /**
     * Login to admin panel.
     */
    const adminCredentials = {
    ...
    await lordPage.getByPlaceholder("Password").fill(adminCredentials.password);
    await lordPage.getByRole("button", { name: "Sign In" }).click();

    /**
     * Create downloadable product.
     */
    const productName = await downloadableOrder(lordPage);

    /**
     * Go to lord for download a product.
     */
    await lordPage.goto("");
    await lordPage.getByLabel("Profile").click();
    await lordPage.getByRole("link", { name: "Profile", exact: true }).click();
    await lordPage
        .getByRole("link", { name: " Downloadable Products " })
        .click();
    const popupPromise = lordPage.waitForEvent('popup').catch(() => null);
    const downloadPromise = lordPage.waitForEvent('download').catch(() => null);
    await lordPage.getByRole("link", { name: productName }).click();
