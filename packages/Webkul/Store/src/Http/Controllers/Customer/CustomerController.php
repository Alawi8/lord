...
use Webkul\Store\Http\Requests\Customer\ProfileRequest;
...
class CustomerController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('store::customers.account.profile.index', compact('customer'));
    }
    ...
    public function edit()
    {
        ...
        return view('store::customers.account.profile.edit', compact('customer'));
    }
    ...
    public function update(ProfileRequest $profileRequest)
    {
        ...
        if (! empty($data['current_password'])) {
            if (Hash::check($data['current_password'], auth()->guard('customer')->user()->password)) {
                ...
                session()->flash('warning', trans('store::app.customers.account.profile.unmatch'));
        ...
        if ($customer = $this->customerRepository->update($data, auth()->guard('customer')->user()->id)) {
            ...
            session()->flash('success', trans('store::app.customers.account.profile.edit-success'));

            return redirect()->route('store.customers.account.profile.index');
        }

        session()->flash('success', trans('store::app.customer.account.profile.edit-fail'));

        return redirect()->back('store.customers.account.profile.edit');
    }
        ...
        try {
            if (Hash::check(request()->input('password'), $customerRepository->password)) {
                ...
                if ($customerRepository->orders->whereIn('status', ['pending', 'processing'])->first()) {
                    session()->flash('error', trans('store::app.customers.account.profile.order-pending'));

                    return redirect()->route('store.customers.account.profile.index');
                } else {
                    $this->customerRepository->delete(auth()->guard('customer')->user()->id);

                    session()->flash('success', trans('store::app.customers.account.profile.delete-success'));

                    return redirect()->route('store.customer.session.index');
                }
            } else {
                session()->flash('error', trans('store::app.customers.account.profile.wrong-password'));

                return redirect()->back();
            }
        } catch (\Exception $e) {
            session()->flash('error', trans('store::app.customers.account.profile.delete-failed'));
    ...
    public function reviews()
    {
        ...
        return view('store::customers.account.reviews.index', compact('reviews'));
    }
}
