...
use Webkul\Store\Http\Requests\Customer\AddressRequest;
...
class AddressController extends Controller
{
    ...
    {
        return view('store::customers.account.addresses.index')->with('addresses', auth()->guard('customer')->user()->addresses);
    }
    ...
    {
        return view('store::customers.account.addresses.create');
    }
    ...
    public function store(AddressRequest $request)
    {
        ...
        session()->flash('success', trans('store::app.customers.account.addresses.create-success'));

        return redirect()->route('store.customers.account.addresses.index');
    }
    ...
    public function edit($id)
    {
        ...
        return view('store::customers.account.addresses.edit')->with('address', $address);
    }
    ...
    public function update($id, AddressRequest $request)
    {
        ...
        if (! $customer->addresses()->find($id)) {
            session()->flash('warning', trans('store::app.customers.account.addresses.security-warning'));
        ...
        session()->flash('success', trans('store::app.customers.account.addresses.edit-success'));
    ...
    public function makeDefault($id)
    {
        ...
        if ($address = $this->customerAddressRepository->find($id)) {
            ...
            session()->flash('success', trans('store::app.customers.account.addresses.default-delete'));
        }
    ...
    public function destroy($id)
    {
        ...
        session()->flash('success', trans('store::app.customers.account.addresses.delete-success'));
