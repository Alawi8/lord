...
class CartController extends APIController
{
    ...
    public function store(): JsonResource
    {
        try {
            ...
            if ($cart) {
                ...
                if (request()->get('is_buy_now')) {
                    Event::dispatch('store.item.buy-now', request()->input('product_id'));

                    return new JsonResource([
                        'data'     => new CartResource(Cart::getCart()),
                        'redirect' => route('store.checkout.onepage.index'),
                        'message'  => trans('store::app.checkout.cart.item-add-to-cart'),
                    ]);
                }
            ...
            return new JsonResource([
                'redirect_uri' => route('store.product_or_category.index', $product->product->url_key),
    ...
    public function destroy(): JsonResource
    {
        ...
        return new JsonResource([
            'data'    => new CartResource(Cart::getCart()),
            'message' => trans('store::app.checkout.cart.success-remove'),
        ]);
    }
    ...
    public function destroySelected(): JsonResource
    {
        ...
        return new JsonResource([
            ...
            'message'  => trans('store::app.checkout.cart.index.remove-selected-success'),
        ]);
    }
    ...
    public function moveToWishlist(): JsonResource
    {
        ...
        return new JsonResource([
            'data'     => new CartResource(Cart::getCart()) ?? null,
            'message'  => trans('store::app.checkout.cart.index.move-to-wishlist-success'),
        ]);
    }
    ...
    public function update(): JsonResource
    {
        try {
            ...
            return new JsonResource([
                'data'    => new CartResource(Cart::getCart()),
                'message' => trans('store::app.checkout.cart.index.quantity-update'),
            ]);
        } catch (\Exception $exception) {
            return new JsonResource([
                'message' => $exception->getMessage(),
    ...
    public function storeCoupon()
    {
        ...
        try {
            if (strlen($couponCode)) {
                ...
                if ($coupon->cart_rule->status) {
                    if (Cart::getCart()->coupon_code == $couponCode) {
                        return (new JsonResource([
                            'data'     => new CartResource(Cart::getCart()),
                            'message'  => trans('store::app.checkout.cart.coupon-already-applied'),
                        ]))->response()->setStatusCode(Response::HTTP_UNPROCESSABLE_ENTITY);
                    }
                    ...
                    if (Cart::getCart()->coupon_code == $couponCode) {
                        return new JsonResource([
                            'data'     => new CartResource(Cart::getCart()),
                            'message'  => trans('store::app.checkout.cart.coupon.success-apply'),
                        ]);
                    }
                }
            }
            ...
            return (new JsonResource([
                'data'    => new CartResource(Cart::getCart()),
                'message' => trans('store::app.checkout.cart.coupon.error'),
    ...
    public function destroyCoupon(): JsonResource
    {
        ...
        return new JsonResource([
            'data'     => new CartResource(Cart::getCart()),
            'message'  => trans('store::app.checkout.cart.coupon.remove'),
        ]);
    }
}
