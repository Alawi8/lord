...
use Webkul\Store\Http\Resources\CartResource;
use Webkul\Store\Http\Requests\Customer\CustomerAddressForm;
...
class OnepageController extends APIController
{
    ...
    public function storeAddress(CustomerAddressForm $request): JsonResource
    {
        ...
        if (
            ...
            return new JsonResource([
                'redirect' => true,
                'data'     => route('store.customer.session.index'),
            ]);
        }
        ...
        if (
            ...
            return new JsonResource([
                'redirect' => true,
                'data'     => route('store.checkout.cart.index'),
            ]);
        }
    ...
    public function storeShippingMethod()
    {
        ...
        if (
            ...
            return response()->json([
                'redirect_url' => route('store.checkout.cart.index'),
    ...
    public function storeOrder(): JsonResource
    {
        ...
        return new JsonResource([
            'redirect'     => true,
            'redirect_url' => route('store.checkout.onepage.success'),
        ]);
    }
    ...
    public function checkMinimumOrder()
    {
        ...
        return response()->json([
            ...
            'message' => ! $status
                ? trans('store::app.checkout.cart.minimum-order-message', [
    ...
    public function validateOrder()
    {
        ...
        if (
            ...
            throw new \Exception(trans('store::app.checkout.cart.suspended-account-message'));
        }
        ...
        if (
            ...
            throw new \Exception(trans('store::app.checkout.cart.inactive-account-message'));
        }

        if (! $cart->checkMinimumOrder()) {
            throw new \Exception(trans('store::app.checkout.cart.minimum-order-message', ['amount' => core()->currency($minimumOrderAmount)]));
        }

        if ($cart->haveStockableItems() && ! $cart->shipping_address) {
            throw new \Exception(trans('store::app.checkout.cart.check-shipping-address'));
        }

        if (! $cart->billing_address) {
            throw new \Exception(trans('store::app.checkout.cart.check-billing-address'));
        }
        ...
        if (
            ...
            throw new \Exception(trans('store::app.checkout.cart.specify-shipping-method'));
        }

        if (! $cart->payment) {
            throw new \Exception(trans('store::app.checkout.cart.specify-payment-method'));
        }
    }
}
