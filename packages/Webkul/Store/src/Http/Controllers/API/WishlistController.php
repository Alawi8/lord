...
use Webkul\Store\Http\Resources\WishlistResource;
...
class WishlistController extends APIController
{
    ...
    public function store(): JsonResource
    {
        ...
        if (! $product) {
            return new JsonResource([
                'message' => trans('store::app.customers.account.wishlist.product-removed'),
            ]);
        }
        ...
        if (! $this->wishlistRepository->findOneWhere($data)) {
            ...
            return new JsonResource([
                'message' => trans('store::app.customers.account.wishlist.success'),
            ]);
        }
        ...
        return new JsonResource([
            'message' => trans('store::app.customers.account.wishlist.removed'),
        ]);
    }
        ...
        try {
            ...
            if ($result) {
                return new JsonResource([
                    ...
                    'message'  => trans('store::app.customers.account.wishlist.moved-success'),
                ]);
            }

            return new JsonResource([
                'redirect' => true,
                'data'     => route('store.product_or_category.index', $wishlistItem->product->url_key),
                'message'  => trans('store::app.checkout.cart.missing-options'),
            ]);
    ...
    public function destroyAll(): JsonResource
    {
        ...
        if (! $success) {
            return new JsonResource([
                'message'  => trans('store::app.customers.account.wishlist.remove-fail'),
            ]);
        }
