{{-- SEO Meta Content --}}
@push('meta')
    <meta name="description" content="@lang('store::app.checkout.cart.index.cart')"/>

    <meta name="keywords" content="@lang('store::app.checkout.cart.index.cart')"/>
@endPush
...
>
    {{-- Page Title --}}
    <x-slot:title>
        @lang('store::app.checkout.cart.index.cart')
    </x-slot>
    ...
    <div class="flex-auto">
        <div class="container px-[60px] max-lg:px-[30px]">
            {{-- Breadcrumbs --}}
            <x-store::breadcrumbs name="cart"></x-store::breadcrumbs>

            <v-cart ref="vCart">
                {{-- Cart Shimmer Effect --}}
                <x-store::shimmer.checkout.cart :count="3"></x-store::shimmer.checkout.cart>
            </v-cart>
        </div>
    </div>
    ...
    @pushOnce('scripts')
                ...
                <template v-else>
                    ...
                    >
                        <div class="grid gap-[25px] flex-1">
                            ...
                            <div class="flex justify-between items-center pb-[10px] border-b-[1px] border-[#E9E9E9] max-sm:block">
                                <div class="flex select-none items-center">
                                    ...
                                    <span class="text-[20px] max-md:text-[22px] max-sm:text-[18px] ml-[10px]">
                                        @{{ "@lang('store::app.checkout.cart.index.items-selected')".replace(':count', selectedItemsCount) }}
                                    </span>
                                </div>
                                ...
                                >
                                    ...
                                    >
                                        @lang('store::app.checkout.cart.index.remove')
                                    </span>
                                    ...
                                    @if (auth()->guard()->check())
                                        ...
                                        >
                                            @lang('store::app.checkout.cart.index.move-to-wishlist')
                                        </span>    
                                    @endif
                                </div>
                            </div>
                                        ...
                                        <div class="grid place-content-start gap-y-[10px]">
                                            ...
                                            >
                                                ...
                                                <div class="">
                                                    ...
                                                    >
                                                        @lang('store::app.checkout.cart.index.see-details')
                            ...
                            {!! view_render_event('bagisto.store.checkout.cart.controls.before') !!}
                            ...
                            <div class="flex flex-wrap gap-[30px] justify-end">
                                <a
                                    ...
                                    href="{{ route('store.home.index') }}"
                                >
                                    @lang('store::app.checkout.cart.index.continue-storeping')
                                </a> 
                                ...
                                <button
                                    ...
                                    @click="update()"
                                >
                                    @lang('store::app.checkout.cart.index.update-cart')
                                </button>
                            </div>

                            {!! view_render_event('bagisto.store.checkout.cart.controls.after') !!}
                            
                        </div>

                        {!! view_render_event('bagisto.store.checkout.cart.summary.before') !!}

                        <!-- Cart Summary -->
                        @include('store::checkout.cart.summary')

                        {!! view_render_event('bagisto.store.checkout.cart.summary.after') !!}
                    ...
                    >
                        ...
                        <p class="text-[20px]">
                            @lang('store::app.checkout.cart.index.empty-product')
                        </p>
                    </div>
                </template>
            </div>
        ...
        <script type="module">
            app.component("v-cart", {
                ...
                methods: {
                    get() {
                        this.$axios.get('{{ route('store.api.checkout.cart.index') }}')
                            .then(response => {
                    ...
                    update() {
                        this.$axios.put('{{ route('store.api.checkout.cart.update') }}', { qty: this.applied.quantity })
                            .then(response => {
                                this.cart = response.data.data;
                    ...
                    removeItem(itemId) {
                        this.$emitter.emit('open-confirm-modal', {
                            agree: () => {
                                this.$axios.post('{{ route('store.api.checkout.cart.destroy') }}', {
                                        '_method': 'DELETE',
                                        'cart_item_id': itemId,
                                    })
                                    .then(response => {
                    ...
                    removeSelectedItems() {
                        this.$emitter.emit('open-confirm-modal', {
                            agree: () => {
                                ...
                                this.$axios.post('{{ route('store.api.checkout.cart.destroy_selected') }}', {
                                        '_method': 'DELETE',
                    ...
                    moveToWishlistSelectedItems() {
                        this.$emitter.emit('open-confirm-modal', {
                            agree: () => {
                                ...
                                this.$axios.post('{{ route('store.api.checkout.cart.move_to_wishlist') }}', {
                                        'ids': selectedItemsIds,
                                    })
                                    .then(response => {
                                        this.cart = response.data.data;
