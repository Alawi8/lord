{!! view_render_event('bagisto.store.checkout.addresses.before') !!}
...
{!! view_render_event('bagisto.store.checkout.addresses.after') !!}
...
@pushOnce('scripts')
    <script type="text/x-template" id="v-checkout-addresses-template">
        ...
        <template v-else>
            <div class="mt-[30px]">
                @include('store::checkout.onepage.addresses.billing')

                @include('store::checkout.onepage.addresses.shipping')
            </div>
        </template>
    </script>
         ...
         app.component('v-checkout-addresses', {
            ...
            methods: {
                ...
                getCustomerAddresses() {
                    if (this.isCustomer) {
                        this.$axios.get("{{ route('api.store.customers.account.addresses.index') }}")
                            .then(response => {
                ...
                getCountries() {
                    this.$axios.get("{{ route('store.api.core.countries') }}")
                        .then(response => {
                ...
                getCountryStates() {
                    this.$axios.get("{{ route('store.api.core.states') }}")
                        .then(response => {
                ...
                handleBillingAddressForm() {
                    if (this.forms.billing.isNew && ! this.forms.billing.address.isSaved) {
                        ...
                        this.$axios.post('{{ route("api.store.customers.account.addresses.store") }}', this.forms.billing.address)
                            .then(response => {
                                this.forms.billing.isNew = false;
                ...
                handleShippingAddressForm() {
                    if (this.forms.shipping.isNew && ! this.forms.shipping.address.isSaved) {
                        ...
                        this.$axios.post('{{ route("api.store.customers.account.addresses.store") }}', this.forms.shipping.address)
                            .then(response => {
                                this.forms.shipping.isNew = false;
                ...
                store() {
                    ...
                    this.$axios.post('{{ route("store.checkout.onepage.addresses.store") }}', {
                            billing: {
                                ...this.forms.billing.address,
