{!! view_render_event('bagisto.store.checkout.payment.method.before') !!}

<v-payment-method ref="vPaymentMethod">
    <x-store::shimmer.checkout.onepage.payment-method/>
</v-payment-method>

{!! view_render_event('bagisto.store.checkout.payment.method.after') !!}
                        ...
                        <x-slot:header>
                            <div class="flex justify-between items-center">
                                <h2 class="text-[26px] font-medium max-sm:text-[20px]">
                                    @lang('store::app.checkout.onepage.payment.payment-method')
                                </h2>
                            </div>
                        </x-slot:header>
        ...
        app.component('v-payment-method', {
            ...
            methods: {
                store(selectedPaymentMethod) {
                    this.$axios.post("{{ route('store.checkout.onepage.payment_methods.store') }}", {
