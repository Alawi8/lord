        ...
        <template v-else>
            <div class="sticky top-[30px] h-max w-[442px] max-w-full pl-[30px] max-lg:w-auto max-lg:max-w-[442px] max-lg:pl-0 ">
                <h2 class="text-[26px] font-medium max-sm:text-[20px]">
                    @lang('store::app.checkout.onepage.summary.cart-summary')
                </h2>
                ...
                <div class="grid gap-[15px] mt-[25px] mb-[30px]">
                    <div class="flex text-right justify-between">
                        <p class="text-[16px] max-sm:text-[14px] max-sm:font-normal">
                            @lang('store::app.checkout.onepage.summary.sub-total')
                        </p>
                    ...
                    >
                        <p class="text-[16px] max-sm:text-[14px] max-sm:font-normal">
                            @lang('store::app.checkout.onepage.summary.tax') (@{{ index }})%
                        </p>
                    ...
                    >
                        <p class="text-[16px]">
                            @lang('store::app.checkout.onepage.summary.delivery-charges')
                        </p>
                    ...
                    >
                        <p class="text-[16px]">
                            @lang('store::app.checkout.onepage.summary.discount-amount')
                        </p>
                    ...
                    @include('store::checkout.onepage.coupon')

                    <div class="flex text-right justify-between">
                        <p class="text-[18px] font-semibold">
                            @lang('store::app.checkout.onepage.summary.grand-total')
                        </p>
                ...
                <template v-if="canPlaceOrder">
                    ...
                    >
                        <button
                            ...
                            @click="placeOrder"
                        >
                            @lang('store::app.checkout.onepage.summary.place-order')    
                        </button>
                        ...
                        >
                            ...
                            @lang('store::app.checkout.onepage.summary.processing')
                        </button>
                    </div>
                </template>
            </div>
        ...
        app.component('v-cart-summary', {
            ...
            methods: {
                placeOrder() {
                    ...
                    this.$axios.post('{{ route('store.checkout.onepage.orders.store') }}')
                        .then(response => {
                            if (response.data.data.redirect) {
                                window.location.href = response.data.data.redirect_url;
                            } else {
                                window.location.href = '{{ route('store.checkout.onepage.success') }}';
                            }
                        })
