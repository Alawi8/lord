            ...
            <p class="text-[16px] max-sm:text-[14px] max-sm:font-medium font-medium">
                                    ...
                                    <x-store::form.control-group>
                                        <div class="p-[30px] bg-white">
                                            <x-store::form.control-group.control
                                                ...
                                                rules="required"
                                                :placeholder="trans('store::app.checkout.onepage.coupon.enter-your-code')"
                                                v-model="code"
                                            >
                                            </x-store::form.control-group.control>
                                    ...
                                    <div class="p-[30px] bg-white mt-[20px]">
                                        <div class="flex justify-between items-center gap-[15px] flex-wrap">
                                            <p class="text-[14px] font-medium text-[#6E6E6E]">
                                                @lang('store::app.checkout.onepage.coupon.sub-total')
                                            </p>
                                            ...
                                            <div class="flex gap-[30px] items-center flex-auto flex-wrap">
                                                ...
                                                <button
                                                    ...
                                                    type="submit"
                                                >
                                                   @lang('store::app.checkout.onepage.coupon.button-title')
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                ...
                >
                    <p class="mr-2 text-[12px]">
                        @lang('store::app.checkout.onepage.coupon.applied')
                    </p>
                    
                    <p 
                        class="text-[16px] font-medium cursor-pointer text-navyBlue"
                        title="@lang('store::app.checkout.onepage.coupon.applied-coupon')"
                    >
                        "@{{ isCouponApplied }}"
                    </p>
                    ...
                    <span 
                        ...
                        title="@lang('store::app.checkout.onepage.coupon.remove')"
                        @click="destroyCoupon"
                    >
                    </span>
                </div>
        ...
        app.component('v-coupon', {
            ...
            methods: {
                applyCoupon(params, { resetForm }) {
                    this.$axios.post("{{ route('store.api.checkout.cart.coupon.apply') }}", params)
                        .then((response) => {
                ...
                destroyCoupon() {
                    this.$axios.delete("{{ route('store.api.checkout.cart.coupon.remove') }}", {
                            '_token': "{{ csrf_token() }}"
                        })
                        .then((response) => {
                            this.$parent.$parent.getOrderSummary();
