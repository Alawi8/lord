                    ...
                    if (imageInput.files && imageInput.files[0]) {
                        if (imageInput.files[0].type.includes('image/')) {
                            if (imageInput.files[0].size <= 2000000) {
                                ...
                                this.$axios.post('{{ route('store.search.upload') }}', formData, {
                                        headers: {
                                    ...
                                    .then(response => {
                                        ...
                                        async function app() {
                                            ...
                                            try {
                                                ...
                                                this.$emitter.emit('add-flash', { type: 'error', message: '@lang('store::app.search.images.index.something-went-wrong')'});
                                            }
                                            ...
                                            window.location.href = `${'{{ route('store.search.index') }}'}?query=${queryString[0]}&image-search=1`;
                                        }
                                ...
                                this.$emitter.emit('add-flash', { type: 'error', message: '@lang('store::app.search.images.index.size-limit-error')'});
                            }
                        } else {
                            imageInput.value = '';

                            this.$emitter.emit('add-flash', { type: 'error', message: '@lang('store::app.search.images.index.only-images-allowed')'});
                        }
                    }
                },
            },
