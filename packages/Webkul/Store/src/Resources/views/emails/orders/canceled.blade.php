@component('store::emails.layout')
    <div style="margin-bottom: 34px;">
        <span style="font-size: 22px;font-weight: 600;color: #121A26">
            @lang('store::app.emails.orders.canceled.title')
        </span> <br>

        <p style="font-size: 16px;color: #5E5E5E;line-height: 24px;">
            @lang('store::app.emails.dear', ['customer_name' => $order->customer_full_name]),👋
        </p>

        <p style="font-size: 16px;color: #5E5E5E;line-height: 24px;">
            {!! __('store::app.emails.orders.canceled.greeting', [
                'order_id' => '<a href="' . route('store.customers.account.orders.view', $order->id) . '" style="color: #2969FF;">#' . $order->increment_id . '</a>',
    ...
    <div style="font-size: 20px;font-weight: 600;color: #121A26">
        @lang('store::app.emails.orders.canceled.summary')
    </div>
        ...
        @if ($order->shipping_address)
            <div style="line-height: 25px;">
                ...
                <div style="font-size: 16px;font-weight: 400;color: #384860;margin-bottom: 40px;">
                    ...
                    @lang('store::app.emails.orders.contact') : {{ $order->billing_address->phone }}
                </div>
        ...
        border-spacing: 0;width: 100%">
            <thead>
                <tr style="color: #121A26;border-top: 1px solid #CBD5E1;border-bottom: 1px solid #CBD5E1;">
                    <th style="text-align: left;padding: 15px">@lang('store::app.emails.orders.sku')</th>
                    <th style="text-align: left;padding: 15px">@lang('store::app.emails.orders.name')</th>
                    <th style="text-align: left;padding: 15px">@lang('store::app.emails.orders.price')</th>
                    <th style="text-align: left;padding: 15px">@lang('store::app.emails.orders.qty')</th>
                </tr>
            </thead>
    ...
    <div style="display: grid;justify-content: end;font-size: 16px;color: #384860;line-height: 30px;padding-top: 20px;padding-bottom: 20px;">
        <div style="display: grid;gap: 100px;grid-template-columns: repeat(2, minmax(0, 1fr));">
            <span>@lang('store::app.emails.orders.subtotal')</span>
        ...
        @if ($order->shipping_address)
            <div style="display: grid;gap: 100px;grid-template-columns: repeat(2, minmax(0, 1fr));">
                <span>@lang('store::app.emails.orders.shipping-handling')</span>
        ...
        @foreach (Webkul\Tax\Helpers\Tax::getTaxRatesWithAmount($order, false) as $taxRate => $taxAmount )
            <div style="display: grid;gap: 100px;grid-template-columns: repeat(2, minmax(0, 1fr));">
                <span>
                    @lang('store::app.emails.orders.tax') {{ $taxRate }} %
                </span>
        ...
        @if ($order->discount_amount > 0)
            <div style="display: grid;gap: 100px;grid-template-columns: repeat(2, minmax(0, 1fr));">
                <span>@lang('store::app.emails.orders.discount')</span>
        ...
        <div style="display: grid;gap: 100px;grid-template-columns: repeat(2, minmax(0, 1fr));font-weight: bold">
            <span>@lang('store::app.emails.orders.grand-total')</span>
