<x-store::layouts.account>
    {{-- Page Title --}}
    <x-slot:title>
        @lang('store::app.customers.account.reviews.title')
    </x-slot>

    {{-- Breadcrumbs --}}
    @section('breadcrumbs')
        <x-store::breadcrumbs name="reviews"></x-store::breadcrumbs>
    @endSection

    <!-- Reviews Vue Component -->
    <v-product-reviews>
        <!-- Reviews Shimmer Effect -->
        <x-store::shimmer.customers.account.reviews :count="4"></x-store::shimmer.customers.account.reviews>
    </v-product-reviews>
    ...
    @pushOnce('scripts')
        <script type="text/x-template" id="v-product-reviews-template">
            <div>
                ...
                {!! view_render_event('bagisto.store.customers.account.reviews.list.before', ['reviews' => $reviews]) !!}
                        ...
                        <div class="max-md:max-w-full">
                            ...
                            @if (! $reviews->isEmpty())
                                ...
                                <div class="grid gap-[20px] mt-[60px] max-1060:grid-cols-[1fr]">
                                    @foreach($reviews as $review)
                                        <a
                                            href="{{ route('store.product_or_category.index', $review->product->url_key) }}"
                            ...
                            @else
                                ...
                                <div class="grid items-center justify-items-center place-content-center w-[100%] m-auto h-[476px] text-center">
                                    ...
                                    <p class="text-[20px]">
                                        @lang('store::app.customers.account.reviews.empty-review')
                                    </p>
                                </div>
                            @endif
                        </div>
                ...
                {!! view_render_event('bagisto.store.customers.account.reviews.list.after', ['reviews' => $reviews]) !!}
        ...
        <script type="module">
            app.component("v-product-reviews", {
                ...
                methods: {
                    get() {
                        this.$axios.get("{{ route('store.customers.account.reviews.index') }}")
                            .then(response => {
