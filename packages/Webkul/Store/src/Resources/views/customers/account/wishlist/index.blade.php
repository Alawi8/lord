<x-store::layouts.account>
    {{-- Page Title --}}
    <x-slot:title>
        @lang('store::app.customers.account.wishlist.page-title')
    </x-slot>

    <!-- Breadcrumbs -->
    @section('breadcrumbs')
        <x-store::breadcrumbs name="wishlist"></x-store::breadcrumbs>
    @endSection

    <!-- Wishlist Vue Component -->
    <v-wishlist-products>
        <!-- Wishlist Shimmer Effect -->
        <x-store::shimmer.customers.account.wishlist :count="4"></x-store::shimmer.customers.account.wishlist>
    </v-wishlist-products>
    ...
    @pushOnce('scripts')
        <script type="text/x-template" id="v-wishlist-products-template">
            <div>
                ...
                {!! view_render_event('bagisto.store.customers.account.wishlist.list.before') !!}
                ...
                <template v-else>
                    <div class="flex justify-between items-center overflow-auto journal-scroll">
                        ...
                        >
                            ...
                            @lang('store::app.customers.account.wishlist.delete-all')
                        </div>
                    </div>
                            ...
                            <div class="grid gap-y-[25px]">
                                ...
                                <div class="flex gap-x-[10px] justify-between pb-[18px] border-b-[1px] border-[#E9E9E9]">
                                    <div class="flex gap-x-[20px]">
                                        <div class="">
                                            <a :href="`{{ route('store.product_or_category.index', '') }}/${item.product.url_key}`">
                                                <!-- Wishlist Item Image -->
                                        ...
                                        <div class="grid gap-y-[10px]">
                                            ...
                                            >
                                                <div class="grid gap-[8px]">
                                                    <div class="">
                                                        ...
                                                        >
                                                            @lang('store::app.customers.account.wishlist.see-details')
                                            ...
                                            <div class="sm:hidden">
                                                ...
                                                >
                                                    @lang('store::app.customers.account.wishlist.remove')
                                                </a>
                                            </div>
                                            ...
                                            <div class="flex gap-[20px] flex-wrap">
                                                ...
                                                >
                                                    @lang('store::app.customers.account.wishlist.move-to-cart')
                                                </button>   
                                            </div>
                                        </div>
                                    </div>
                    ...
                    >
                        ...
                        <p class="text-[20px]">
                            @lang('store::app.customers.account.wishlist.empty')
                        </p>
                    </div>
                </template>

                {!! view_render_event('bagisto.store.customers.account.wishlist.list.after') !!}
        ...
        <script type="module">
            app.component("v-wishlist-products", {
                ...
                methods: {
                    get() {
                        this.$axios.get("{{ route('store.api.customers.account.wishlist.index') }}")
                            .then(response => {
                    ...
                    remove(id) {
                        this.$axios.delete(`{{ route('store.api.customers.account.wishlist.destroy', '') }}/${id}`)
                            .then(response => {
                    ...
                    removeAll() {
                        this.$axios.delete("{{ route('store.api.customers.account.wishlist.destroy_all') }}")
                            .then(response => {
                                this.wishlist = [];
                    ...
                    moveToCart(id) {
                        let url = `{{ route('store.api.customers.account.wishlist.move_to_cart', ':wishlist_id:') }}`;
