{!!view_render_event('bagisto.store.categories.view.filters.before') !!}
...
<div v-if="! isMobile">
    ...
    >
        ...
        <x-store::shimmer.categories.filters/>
    </v-filters>
</div>
    ...
    >
        ...
        <x-slot:toggle>
            ...
            >
                ...
                @lang('store::app.categories.filters.filter')
            </div>
        </x-slot:toggle>
        ...
        <x-slot:header>
            <div class="flex justify-between items-center pb-[20px] border-b-[1px] border-[#E9E9E9]">
                <p class="text-[18px] font-semibold">
                    @lang('store::app.categories.filters.filters')
                </p>
                ...
                >
                    @lang('store::app.categories.filters.clear-all')
                </p>
            </div>
        </x-slot:header>
    ...
    >
        ...
        <x-slot:toggle>
            ...
            >
                ...
                @lang('store::app.categories.filters.sort')
            </div>
        </x-slot:toggle>
...
{!!view_render_event('bagisto.store.categories.view.filters.after') !!}
...
@pushOnce('scripts')
    ...
    <script type="text/x-template" id="v-price-filter-template">
        <div>
            ...
            <template v-if="isLoading">
                <x-store::shimmer.range-slider/>
            </template>

            <template v-else>
                <x-store::range-slider
                    ::key="refreshKey"
                    default-type="price"
    ...
    <script type='module'>
        app.component('v-filters', {
            ...
            methods: {
                getFilters() {
                    this.$axios.get('{{ route("store.api.categories.attributes") }}', {
                            params: { 
        ...
        app.component('v-price-filter', {
            ...
            methods: {
                getMaxPrice() {
                    this.$axios.get('{{ route("store.api.categories.max_price", $category->id ?? '') }}')
                        .then((response) => {
                            this.isLoading = false;
