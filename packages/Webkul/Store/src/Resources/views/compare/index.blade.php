{{-- SEO Meta Content --}}
@push('meta')
    <meta name="description" content="@lang('store::app.compare.title')"/>

    <meta name="keywords" content="@lang('store::app.compare.title')"/>
@endPush

<x-store::layouts>
    {{-- Page Title --}}
    <x-slot:title>
        @lang('store::app.compare.title')
    </x-slot:title>

    {{-- Breadcrumb --}}
    <div class="flex justify-center mt-[20px] max-lg:hidden">
		<div class="flex gap-x-[10px] items-center">
            <x-store::breadcrumbs name="compare"></x-store::breadcrumbs>
		</div>
	</div>
    ...
    @pushOnce('scripts')
        <script type="text/x-template" id="v-compare-template">
            <div>
                ...
                {!! view_render_event('bagisto.store.customers.account.compare.view.before') !!}
                ...
                <div v-if="! isLoading">
                    <div class="flex justify-between items-center">
                        ...
                        >
                            ...
                            @lang('store::app.compare.delete-all')
                        </div>
                    </div>
                            ...
                            >
                                ...
                                <div class="flex gap-[12px] border-l-[1px] border-[#E9E9E9] max-sm:border-0">
                                    ...
                                    >
                                        ...
                                        <x-store::products.card class="min-w-[311px] max-w-[311px] pt-0 pr-0 p-[20px] max-sm:pl-0"></x-store::products.card>
                                    </div>
                                </div>
                            </div>
                    ...
                    >
                        ...
                        <p class="text-[20px]">@lang('store::app.compare.empty-text')</p>
                    </div>
                </div>
                ...
                {!! view_render_event('bagisto.store.customers.account.compare.view.after') !!}
            ...
            app.component("v-compare", {
                ...
                methods: {
                    getItems() {
                        ...
                        this.$axios.get("{{ route('store.api.compare.index') }}", {
                                params: {
                    ...
                    remove(productId) {
                        this.$emitter.emit('open-confirm-modal', {
                            agree: () => {
                                ...
                                this.$axios.post("{{ route('store.api.compare.destroy') }}", {
                                        '_method': 'DELETE',
                        ...
                        this.$emitter.emit('open-confirm-modal', {
                            agree: () => {
                                if (! this.isCustomer) {
                                    ...
                                    this.$emitter.emit('add-flash', { type: 'success', message:  "@lang('store::app.compare.remove-all-success')" });

                                    return;
                                }
                                
                                this.$axios.post("{{ route('store.api.compare.destroy_all') }}", {
                                        '_method': 'DELETE',
                                    })
                                    .then(response => {
                                        this.items = [];
