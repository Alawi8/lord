<x-table>
    <x-table.thead>
        <x-table.tr>
            <x-table.th>Column 1</x-table.th>
            <x-table.th>Column 2</x-table.th>
            <x-table.th class="action">Column 3</x-table.th>
        </x-table.tr>
    </x-table.thead>

    <x-table.tbody>
        <x-table.tr>
            <x-table.td>Row 1 Value 1</x-table.td>
            <x-table.td>Row 1 Value 3</x-table.td>
            <x-table.td class="w-[50px]">Row 1 Value 3</x-table.td>
        </x-table.tr>

        <x-table.tr>
            <x-table.td>Row 2 Value 1</x-table.td>
            <x-table.td>Row 2 Value 3</x-table.td>
            <x-table.td class="action">Row 2 Value 3</x-table.td>
        </x-table.tr>
    </x-table.tbody>
</x-table>

<x-flash-group></x-flash-group>

<x-panel>
    <x-slot:header>Panel Title</x-slot:header>

    <x-slot:body>Panel Body</x-slot:body>
</x-panel>

{{-- default product listing --}}
<x-store::products.carousel
    title="Men's Collections"
    :src="route('store.api.products.index')"
    :navigation-link="route('store.home.index')"
>
</x-store::products.carousel>

{{-- category product listing --}}
<x-store::products.carousel
    title="Men's Collections"
    :src="route('store.api.products.index', ['category_id' => 1])"
    :navigation-link="route('store.home.index')"
>
</x-store::products.carousel>

{{-- featured product listing --}}
<x-store::products.carousel
    title="Men's Collections"
    :src="route('store.api.products.index', ['featured' => 1])"
    :navigation-link="route('store.home.index')"
>
</x-store::products.carousel>

{{-- new product listing --}}
<x-store::products.carousel
    title="Men's Collections"
    :src="route('store.api.products.index', ['new' => 1])"
    :navigation-link="route('store.home.index')"
>
</x-store::products.carousel>

{{-- basic/traditional form  --}}
<x-store::form action="">
    <x-store::form.control-group>
        <x-store::form.control-group.label>
            Email
        </x-store::form.control-group.label>

        <x-store::form.control-group.control
            type="email"
            name="email"
            value=""
            rules="required|email"
            label="Email"
            placeholder="email@example.com"
        >
        </x-store::form.control-group.control>

        <x-store::form.control-group.error
            control-name="email"
        >
        </x-store::form.control-group.error>
    </x-store::form.control-group>
</x-store::form>

{{-- customized/ajax form --}}
<x-store::form
    v-slot="{ meta, errors, handleSubmit }"
    as="div"
>
    <form @submit="handleSubmit($event, callMethodInComponent)">
        <x-store::form.control-group>
            <x-store::form.control-group.label>
                Email
            </x-store::form.control-group.label>

            <x-store::form.control-group.control
                type="email"
                name="email"
                :value="old('email')"
                rules="required"
                label="Email"
                placeholder="email@example.com"
            >
            </x-store::form.control-group.control>

            <x-store::form.control-group.error
                control-name="email"
            >
            </x-store::form.control-group.error>
        </x-store::form.control-group>

        <button>Submit</button>
    </form>
</x-store::form>

{{-- tabs --}}
<x-store::tabs>
    <x-store::tabs.item
        title="Tab 1"
    >
        Tab 1 Content
    </x-store::tabs.item>

    <x-store::tabs.item
        title="Tab 2"
    >
        Tab 2 Content
    </x-store::tabs.item>
</x-store::tabs>

{{-- accordion --}}
<x-store::accordion>
    <x-slot:header>
        Accordion Header
    </x-slot:header>

    <x-slot:content>
        Accordion Content
    </x-slot:content>
</x-store::accordion>

{{-- modal --}}
<x-store::modal>
    <x-slot:toggle>
        Modal Toggle
    </x-slot:toggle>

    <x-slot:header>
        Modal Header
    </x-slot:header>

    <x-slot:content>
        Modal Content
    </x-slot:content>
</x-store::modal>

{{-- drawer --}}
<x-store::drawer>
    <x-slot:toggle>
        Drawer Toggle
    </x-slot:toggle>

    <x-slot:header>
        Drawer Header
    </x-slot:header>

    <x-slot:content>
        Drawer Content
    </x-slot:content>
</x-store::drawer>

{{-- dropdown --}}
<x-store::dropdown>
    <x-slot:toggle>
        Toogle
    </x-slot:toggle>

    <x-slot:content>
        Content
    </x-slot:content>
</x-store::dropdown>
