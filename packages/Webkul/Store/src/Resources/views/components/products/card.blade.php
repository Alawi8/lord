    ...
    <script type="text/x-template" id="v-product-card-template">
        ...
        >
            <div class="relative overflow-hidden group max-w-[291px] max-h-[300px] rounded-[4px]">
                <a
                    :href="`{{ route('store.product_or_category.index', '') }}/${product.url_key}`"
                ...
                <div class="action-items bg-black">
                    ...
                    >
                        @lang('store::app.components.products.card.sale')
                    </p>
                    ...
                    >
                        @lang('store::app.components.products.card.new')
                    </p>
                    ...
                    <div class="group-hover:bottom-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
                        ...
                        <button
                            ...
                            @click="addToCart()"
                        >
                            @lang('store::app.components.products.card.add-to-cart')
                        </button>
                    </div>
                </div>
            </div>
        ...
        >
            <div class="relative max-w-[250px] max-h-[258px] overflow-hidden group"> 
                <a :href="`{{ route('store.product_or_category.index', '') }}/${product.url_key}`">
                    <x-store::media.images.lazy
            ...
            <div class="grid gap-[15px] content-start"> 
                ...
                <p class="text-[14px] text-[#6E6E6E]" v-if="! product.avg_ratings">
                    @lang('store::app.components.products.card.review-description')
                </p>
    ...
    <script type="module">
        app.component('v-product-card', {
            ...
            methods: {
                ...
                addToCompare(productId) {
                    ...
                    if (items.length) {
                        if (! items.includes(productId)) {
                            ...
                            this.$emitter.emit('add-flash', { type: 'success', message: "@lang('store::app.components.products.card.add-to-compare')" });
                        } else {
                            this.$emitter.emit('add-flash', { type: 'warning', message: "@lang('store::app.components.products.card.already-in-compare')" });
                        }
                    } else {
                ...
                addToCart() {
                    this.$axios.post('{{ route("store.api.checkout.cart.store") }}', {
                            'quantity': 1,
                            'product_id': this.product.id,
                        })
                        .then(response => {
