{!! view_render_event('bagisto.store.layout.footer.before') !!}

{{--
    The category repository is injected directly here because there is no way
    to retrieve it from the view composer, as this is an anonymous component.
--}}
@inject('themeCustomizationRepository', 'Webkul\\Store\\Repositories\\ThemeCustomizationRepository')
...
<footer class="mt-[36px] bg-lightOrange  max-sm:mt-[30px]">
            ...
            @if(core()->getConfigData('customer.settings.newsletter.subscription'))
                <div class="grid gap-[10px]">
                    <p class="max-w-[288px] leading-[45px] text-[30px] italic text-navyBlue">
                        @lang('store::app.components.layouts.footer.newsletter-text')
                    </p>

                    <p class="text-[12px]">
                        @lang('store::app.components.layouts.footer.subscribe-stay-touch')
                    </p>

                    <x-store::form
                        :action="route('store.subscription.store')"
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                @lang('store::app.components.layouts.footer.subscribe')
                            </button>
                        </div>
                    </x-store::form>
                </div>
    ...
    <div class="flex justify-between  px-[60px] py-[13px] bg-[#F1EADF]">
        <p class="text-[14px] text-[#4D4D4D]">
            @lang('store::app.components.layouts.footer.footer-text')
        </p>
    </div>
</footer>

{!! view_render_event('bagisto.store.layout.footer.after') !!}
