<div class="w-full flex justify-between min-h-[78px] px-[60px] border border-t-0 border-b-[1px] border-l-0 border-r-0 max-1180:px-[30px]">
    ...
    <div class="flex gap-x-[35px] items-center max-lg:gap-x-[30px] max-[1100px]:gap-x-[25px]">
        ...
        <form
            action="{{ route('store.search.index') }}"
        ...
        >
            <label
                ...
                class="sr-only"
            >
                @lang('store::app.components.layouts.header.search')
            </label>
            ...
            <div class="relative w-full">
                ...
                <input
                    ...
                    placeholder="@lang('store::app.components.layouts.header.search-text')"
                    required
                >

                @if (core()->getConfigData('general.content.store.image_search'))
                    @include('store::search.images.index')
                @endif
            </div>
        </form>
        ...
        <div class="flex gap-x-[35px] mt-[5px] max-lg:gap-x-[30px] max-[1100px]:gap-x-[25px]">
            ...
            @if(core()->getConfigData('general.content.store.compare_option'))
                <a
                    href="{{ route('store.compare.index') }}"
                    aria-label="Compare"
                >
            ...
            {{-- Mini cart --}}
            @include('store::checkout.cart.mini-cart')

            {{-- user profile --}}
            <x-store::dropdown position="bottom-{{ core()->getCurrentLocale()->direction === 'ltr' ? 'right' : 'left' }}">
                <x-slot:toggle>
                ...
                @auth('customer')
                    <x-slot:content class="!p-[0px]">
                        <div class="grid gap-[10px] p-[20px] pb-0">
                            <p class="text-[20px] font-dmserif">
                                @lang('store::app.components.layouts.header.welcome')’
                        ...
                        <div class="grid gap-[4px] mt-[10px] pb-[10px]">
                            ...
                            >
                                @lang('store::app.components.layouts.header.profile')
                            </a>
                            ...
                            >
                                @lang('store::app.components.layouts.header.orders')
                            </a>
                            ...
                            @if (core()->getConfigData('general.content.store.wishlist_option'))
                                ...
                                >
                                    @lang('store::app.components.layouts.header.wishlist')
                                </a>
                            @endif
                            ...
                            @auth('customer')
                                <x-store::form
                                    method="DELETE"
                                    action="{{ route('store.customer.session.destroy') }}"
                                    id="customerLogout"
                                >
                                </x-store::form>
                                ...
                                <a
                                    class="px-5 py-2 text-[16px] hover:bg-gray-100 cursor-pointer"
                                    href="{{ route('store.customer.session.destroy') }}"
                                    onclick="event.preventDefault(); document.getElementById('customerLogout').submit();"
                                >
                                    @lang('store::app.components.layouts.header.logout')
                                </a>
                            @endauth
                        </div>
                    </x-slot:content>
...
@pushOnce('scripts')
    ...
    <script type="module">
        app.component('v-desktop-category', {
            ...
            methods: {
                get() {
                    this.$axios.get("{{ route('store.api.categories.tree') }}")
                        .then(response => {
