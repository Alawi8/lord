...
class DownloadableProductDataGrid extends DataGrid
{
    ...
    public function prepareColumns()
    {
        $this->addColumn([
            'index'      => 'increment_id',
            'label'      => trans('store::app.customer.account.downloadable_products.order-id'),
            'type'       => 'string',
            'searchable' => false,
            'sortable'   => true,
            'filterable' => true,
        ...
        $this->addColumn([
            'index'      => 'product_name',
            'label'      => trans('store::app.customer.account.downloadable_products.name'),
            'type'       => 'string',
            'searchable' => true,
            'sortable'   => true,
            'filterable' => true,
            'closure'    => function ($value) {
                ...
                return $value->product_name . ' ' . '<a href="' . route('store.customer.downloadable_products.download', $value->id) . '" target="_blank">' . $value->name . '</a>';
            },
        ]);

        $this->addColumn([
            'index'      => 'created_at',
            'label'      => trans('store::app.customer.account.downloadable_products.date'),
            'type'       => 'datetime',
            'searchable' => false,
            'sortable'   => true,
            'filterable' => true,
        ...
        $this->addColumn([
            'index'      => 'status',
            'label'      => trans('store::app.customer.account.downloadable_products.status'),
            'type'       => 'string',
            'searchable' => false,
            'sortable'   => true,
            'filterable' => true,
            'closure'    => function ($value) {
                if ($value->status == 'pending') {
                    return '<span class="badge badge-md badge-warning">' . trans('store::app.customer.account.downloadable_products.pending') . '</span>';
                } elseif ($value->status == 'available') {
                    return '<span class="badge badge-md badge-success">' . trans('store::app.customer.account.downloadable_products.available') . '</span>';
                } elseif ($value->status == 'expired') {
                    return '<span class="badge badge-md badge-danger">' . trans('store::app.customer.account.downloadable_products.expired') . '</span>';
                }
            },
        ]);
        ...
        $this->addColumn([
            ...
            'label'      => trans('store::app.customer.account.downloadable_products.remaining-downloads'),
            'type'       => 'datetime',
            'searchable' => false,
            'sortable'   => true,
            'filterable' => false,
            'closure'    => function ($value) {
                if (! $value->download_bought) {
                    return trans('store::app.customer.account.downloadable_products.unlimited');
                }
